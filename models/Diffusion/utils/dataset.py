...
#                     pointclouds.append(torch.from_numpy(f[synsetid][split][...]))
        if self.split=='train':
            ...
            pointclouds=torch.from_numpy(np.load('/home/daia/diffusion-point-cloud/eval_results/data/testset_chair.npy')[1])
        elif self.split=='full':
            #print("yes")
            pointclouds=torch.from_numpy(np.load('diffusion-point-cloud/chair_full.npy')[0:20])
            pointclouds[:,:,[0,1,2]] = pointclouds[:,:,[1,2,0]] 
            pointclouds[:,:,2]*=-1
        #print(torch.is_tensor(pointclouds))
        print(pointclouds.size())
        all_points = pointclouds#torch.cat(pointclouds, dim=0) # (B, N, 3)
...
#                     yield torch.from_numpy(pc), j, cate_name
        ...
        if self.split=='train':
            ...
            pointclouds=np.load('/home/daia/diffusion-point-cloud/eval_results/data/testset_chair.npy')
            f=pointclouds
            
        elif self.split=='full':
            pointclouds=np.load('AAE/chair_full_normalized.npy')[0:20]# enter the npy arrays for latent space [128, 2048, 3]
            f=pointclouds
